...
export const coreStylesByGenre: Record<string, CoreStyle[]> = {
  "犯罪悬疑": [
    ...
    core("犯罪悬疑", "crime-hunt", "猫鼠追凶", "把调查者与对手都拍成明星角色，以追逐不断交换优势。", "双雄对峙", "猎手换位", "明星角色", ["market", "character", "story"], ["safe"], { castingCostMultiplier: 1.06, openingPower: 3, starPowerMultiplier: 1.12, awardActing: 2 }, ["核心主演成本 +6%", "明星开画权重提高", "表演评审 +2"]),
  ],
  "都市爱情": [
    core("都市爱情", "love-concept", "高概念爱情", "用一句话能讲清的奇遇，制造浪漫传播钩子。", "爱情设定", "命运回环", "非线性结构", ["market", "original", "story"], ["safe"], { openingPower: 3, wordOfMouth: -1, awardDirector: 1, retention: .005 }, ["开画话题 +3", "导演评审 +1", "若收束不足口碑承压"]),
    core("都市爱情", "love-reality", "现实关系", "把住房、工作与迁徙写进感情选择。", "现实压力", "生活剖面", "现实议题", ["character", "balanced", "original"], ["spectacle"], { openingPower: -2, wordOfMouth: 2, awardPicture: 3, libraryMultiplier: 1.08 }, ["开画 -2、口碑 +2", "最佳影片评审 +3", "片库长尾 +8%"]),
    core("都市爱情", "love-healing", "治愈成长", "让关系成为两个人重新面对自己的契机。", "共同成长", "温柔回声", "开放结局", ["emotional", "character", "safe"], ["risky"], { wordOfMouth: 2, retention: .012, awardActing: 2, openingPower: -1 }, ["口碑与长线留存提高", "表演评审 +2", "开画略慢"]),
  ],
  "科幻冒险": [
    ...
    core("科幻冒险", "scifi-spectacle", "奇观冒险", "以从未见过的世界和大场面兑现银幕价值。", "未知奇观", "银幕跃迁", "奇观驱动", ["spectacle", "market", "original"], ["safe"], { budgetCostMultiplier: 1.08, contentQuality: 2, openingPower: 4, starPowerMultiplier: .92 }, ["制作成本 +8%", "质量 +2、开画 +4", "单明星作用略降"]),
  ...
  "动作战争": [
    core("动作战争", "action-tactical", "特种行动", "以空间、装备与时间差构成可读的战术博弈。", "战术协同", "零点行动", "封闭空间", ["story", "market", "balanced"], ["emotional"], { budgetCostMultiplier: .94, contentQuality: 1, openingPower: 2, wordOfMouth: 1 }, ["制作成本 -6%", "质量 +1、开画 +2", "强调团队默契"]),
    ...
    core("动作战争", "action-revenge", "孤胆复仇", "围绕一个不可替代的明星角色推进高强度行动。", "复仇执念", "孤锋燃点", "明星角色", ["character", "market", "spectacle"], ["balanced"], { castingCostMultiplier: 1.08, starPowerMultiplier: 1.14, openingPower: 4, awardActing: 2, wordOfMouth: -1 }, ["核心主演成本 +8%", "开画 +4", "表演评审 +2但口碑 -1"]),
  ],
  "合家欢喜剧": [
    core("合家欢喜剧", "comedy-farce", "疯狂闹剧", "让误会与行动后果层层升级，最终在同一场面爆炸。", "连锁误会", "失控狂欢", "强反转", ["market", "story", "risky"], ["emotional"], { openingPower: 4, wordOfMouth: -1, retention: -.005, starPowerMultiplier: .95 }, ["开画讨论 +4", "伏笔不足会伤口碑", "单明星作用略降"]),
    ...
    core("合家欢喜剧", "comedy-road", "公路喜剧", "让一段不断变化的旅程重组临时家庭。", "旅途搭档", "沿途变奏", "系列化世界观", ["balanced", "market", "character"], ["safe"], { budgetCostMultiplier: .97, openingPower: 2, libraryMultiplier: 1.12, retention: .006 }, ["制作成本 -3%、开画 +2", "片库长尾 +12%", "适合系列开发"]),
  ],
  "历史传记": [
    core("历史传记", "history-power", "权谋传记", "让制度与选择互相绞紧，以胜负展示人物代价。", "权力棋局", "暗线合围", "非线性结构", ["story", "original", "character"], ["safe"], { contentQuality: 1, wordOfMouth: 1, awardDirector: 3, openingPower: -1 }, ["质量/口碑 +1", "导演评审 +3", "开画略慢"]),
    ...
    core("历史传记", "history-portrait", "人物史诗", "把时代压在一个人的选择上，塑造不可替代的表演角色。", "命运肖像", "一生回望", "明星角色", ["character", "emotional", "story"], ["spectacle"], { castingCostMultiplier: 1.05, starPowerMultiplier: 1.08, awardActing: 4, wordOfMouth: 1, openingPower: 1 }, ["核心主演成本 +5%", "表演评审 +4", "口碑/开画 +1"]),
  ],
};
...
export const scriptConnections: ScriptConnection[] = [
  { id: "clue-turn", name: "反转闭环", keywords: ["严密因果", "强反转"], trait: "强反转", downstream: { openingPower: 2, wordOfMouth: 1, retention: -.005 }, effects: ["开画讨论 +2、口碑 +1", "反转前置使长线留存 -0.5%"] },
  { id: "people-heart", name: "众生余震", keywords: ["关系网络", "情绪余震"], trait: "群像叙事", downstream: { ensemble: true, budgetCostMultiplier: 1.03, starPowerMultiplier: .9, awardPicture: 1, awardDirector: 1 }, effects: ["启用群像班底、影片/导演评审 +1", "班底成本 +3%、明星开画权重下降"] },
  { id: "character-heart", name: "人物回响", keywords: ["人物弧光", "情绪余震"], trait: "开放结局", downstream: { openingPower: -1, wordOfMouth: 2, retention: .008 }, effects: ["口碑 +2、长线留存 +0.8%", "开画话题 -1"] },
  { id: "world-logic", name: "规则回收", keywords: ["陌生规则", "严密因果"], trait: "系列化世界观", downstream: { openingPower: -1, wordOfMouth: 1, libraryMultiplier: 1.06 }, effects: ["口碑 +1、片库长尾 +6%", "设定解释使开画 -1"] },
  { id: "market-spectacle", name: "银幕名片", keywords: ["大众钩子", "奇观场面"], trait: "奇观驱动", downstream: { budgetCostMultiplier: 1.06, openingPower: 3, wordOfMouth: -1 }, effects: ["开画话题 +3", "制作成本 +6%、人物口碑 -1"] },
  { id: "market-character", name: "明星角色", keywords: ["大众钩子", "人物弧光"], trait: "明星角色", downstream: { castingCostMultiplier: 1.04, starPowerMultiplier: 1.08, openingPower: 2, awardActing: 1 }, effects: ["明星开画权重提高、表演评审 +1", "核心主演成本 +4%"] },
  { id: "closure-heart", name: "完整余味", keywords: ["完整收束", "情绪余震"], trait: "现实议题", downstream: { openingPower: -1, wordOfMouth: 1, awardPicture: 1, libraryMultiplier: 1.03 }, effects: ["口碑/影片评审 +1、长尾 +3%", "克制收束使开画 -1"] },
];
...
export function summarizeScriptDownstream(effect: ScriptDownstream) {
  ...
  if (effect.openingPower) summary.push(`开画 ${signed(effect.openingPower)}`);
  if (effect.retention) summary.push(`长线留存 ${signed(effect.retention * 100)}%`);
  if (effect.starPowerMultiplier !== 1) summary.push(`明星开画权重 ×${effect.starPowerMultiplier.toFixed(2)}`);
