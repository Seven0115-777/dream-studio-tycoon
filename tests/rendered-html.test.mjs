...
test("keeps the mobile shell separate from simulation systems", async () => {
  ...
  assert.match(page, /核心优势 · \{option\.coreAdvantages\}/);
  assert.match(page, /构筑代价 · \{option\.coreCosts\}/);
  assert.match(page, /option\.routeFunction === "core"/);
  assert.match(page, /<em>收益 · \{option\.upside\}<\/em><em>关系/);
